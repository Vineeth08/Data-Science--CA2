...
# Q-Q of quantile deviance residuals
mgcv::qq.gam(model,pch=1, main = "Q-Q plot for residuals")
...
# Calculating accuracy and mean classification error for logistic regression
...
actuals_predictions <- data.frame(cbind(Actuals = testing_data$`Chances_of_Heart Attack`, 
                                        Predicted = fitted.results1))


head(actuals_predictions)

write.csv(actuals_predictions, file = "actual predictions.csv")

# Calculating the correlation accuracy
correlation_accuracy <- cor(actuals_predictions)
correlation_accuracy

print(paste("accuracy for the model", mean(fitted.results1 == testing_data$`Chances_of_Heart Attack`)))

# accuracy for the model is 0.8666 or 86.67%
...
new_df <- cbind(df, predicted_chances)
new_df
